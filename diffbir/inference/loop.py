...
class InferenceLoop:
    ...
    def load_cldm(self) -> None:
        cldm_cfg = OmegaConf.load("configs/inference/cldm.yaml")
        if self.args.identity_ckpt:
            if self.args.task != "face":
                raise ValueError("stage-1 identity attention currently supports face only")
            Config.attn_mode = AttnMode.SDP
            cldm_cfg.params.unet_cfg.identity_context_dim = 1024
            cldm_cfg.params.identity_projector_cfg = {
                "embedding_dim": 512,
                "hidden_dim": 1024,
                "context_dim": 1024,
                "num_tokens": 4,
            }
        self.cldm: ControlLDM = instantiate_from_config(cldm_cfg)
