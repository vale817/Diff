...
from ..model import build_identity_encoder
...
class BFRInferenceLoop(InferenceLoop):
    ...
    def load_pipeline(self) -> None:
        ...
        if self.args.identity_ckpt:
            if not self.args.identity_encoder:
                raise ValueError("--identity_encoder is required with --identity_ckpt")
            identity_encoder = build_identity_encoder(
                self.args.identity_encoder
