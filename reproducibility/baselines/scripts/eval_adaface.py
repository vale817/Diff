...
def compute_cosines(
    ...
    scores = []
    for start in tqdm(range(0, len(restored_paths), batch_size), desc="AdaFace cosine"):
...
def main() -> None:
    parser = ArgumentParser(
        description=(
            "Evaluate identity preservation with AdaFace cosine similarity. "
            "Images are matched by the same relative file path without extension."
        )
    )
    parser.add_argument("--adaface_encoder", required=True, help="AdaFace TorchScript model path.")
    ...
    parser.add_argument(
        "--output_csv",
        default="",
        help="Optional path to save per-image AdaFace cosine results.",
    )
    ...
    print(f"mean AdaFace cosine: {mean(restored_scores):.6f}")
    ...
    if baseline_scores is not None and deltas is not None:
        ...
        print(f"baseline mean AdaFace cosine: {mean(baseline_scores):.6f}")
        print(f"mean delta AdaFace: {mean(deltas):+.6f}")
    ...
    if args.output_csv:
        ...
        with output_path.open("w", newline="") as csv_file:
            fieldnames = ["key", "adaface_cosine"]
            if baseline_scores is not None:
                fieldnames += ["baseline_adaface_cosine", "delta_adaface"]
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            writer.writeheader()
            for index, key in enumerate(keys):
                row = {
                    "key": key,
                    "adaface_cosine": restored_scores[index],
                }
                if baseline_scores is not None and deltas is not None:
                    row["baseline_adaface_cosine"] = baseline_scores[index]
                    row["delta_adaface"] = deltas[index]
                writer.writerow(row)
